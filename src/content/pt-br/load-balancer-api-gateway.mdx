...
Ele é o ponto de entrada único de todas as requisições externas para os seus serviços. Em vez de o cliente saber o endereço de cada microserviço, como `/users`, `/orders`, `/products`, ele só conhece o gateway. O gateway recebe a requisição, identifica pra qual serviço ela pertence e faz o roteamento.

Além do roteamento, um API Gateway pode centralizar várias responsabilidades: autenticação, rate limiting, logging, CORS, transformação de requests. Você para de repetir essas configurações em cada serviço e coloca tudo num lugar só.
...
A estrutura tem três serviços simulados (chamados de `users`, `products` e `orders`), cada um rodando em múltiplas instâncias via Docker Compose. O NGINX age como API Gateway na entrada e como load balancer na frente de cada conjunto de instâncias.
...
Quando você bate em `localhost:8080/users/health`, o NGINX captura essa requisição, vê que o path começa com `/users/` e distribui entre as três instâncias de `users` usando Round Robin por padrão.
...
(Nesse caso, `users-1` recebe três vezes mais requisições do que os outros dois. Útil quando uma instância tem mais recurso alocado.)
...
Com essa configuração, se `users-1` falhar três vezes em 30 segundos, o NGINX para de mandar requisições pra ele por 30 segundos. Depois tenta de novo. Se o servidor voltou, volta pra rotação. Se ainda estiver falhando, o ciclo continua.

Pra testar isso no projeto do repositório, você pode derrubar uma das instâncias com `docker stop <container>` e observar que as requisições continuam sendo respondidas normalmente pelas outras instâncias.
...
Caso queira dar uma olhada no repositório que tenho utilizado como estudo: [github.com/Viilih/nginx-resources](https://github.com/Viilih/nginx-resources/tree/main/02-api-gateway-lb). Sobe o Docker Compose, derruba uma instância, muda o algoritmo do upstream e observa o comportamento
